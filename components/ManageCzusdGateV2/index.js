...
export default function ManageCzusdGateV2({
  ...
  const usdtAllowance = useTokenAllowance(
    ADDRESS_USDT,
    account,
    ADDRESS_CZUSDGATE_V2
  );

  const { value: sellFeeBasisResult, error: sellFeeBasisError } =
    useCall({
  ...
  const { state: stateApproveUsdt, send: sendApproveUsdt } =
    useContractFunction(
            ...
            {usdtAllowance?.lt(usdtBal ?? 0) ? (
              ...
              >
                <div className="mb-5">
                  ...
                  <p className="mb-2">
                    To use the USDT to CZUSD portion of CzusdGate, you need to
                    approve the CzusdGate address{' '}
                    <code>{ADDRESS_CZUSDGATE_V2}</code> to use your USDT. You
                    can use the button below.
                  </p>
                  <button
                    onClick={() =>
                      sendApproveUsdt(ADDRESS_CZUSDGATE_V2, MaxUint256)
                    }
              ...
              <>
                ...
                >
                  <h3 className="is-size-4">USDT to CZUSD</h3>
                  <p>
                    Swap your USDT and receive CZUSD. No daily limit. Max is
                    CZUSD balance of contract. No fee.
                  </p>
                  <InputTokenEther
                  ...
                  <button
                    onClick={() =>
                      sendUsdtIn(parseEther(inputEtherUsdt.toString()), account)
                    }
                    className="button has-background-grey-lighter is-fullwidth"
                  >
                    Swap
                  ...
                  <p>
                    You will get{' '}
                    {(
                      BigNumber.from(inputEtherUsdt ?? 0)
                        .mul(100)
                        .toNumber() / 100
                    ).toFixed(2)}{' '}
                    CZUSD immediately.
