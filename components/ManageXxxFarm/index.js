...
export default function ManageXxxFarm({ account, library, farm, xxxFarmsSettings, xxxFarmsPoolInfo, xxxFarmsPendingXxx, xxxFarmsUserInfo, lpInfo, accountLpBal, xxxPrice, czusdPrice, address_xxx_master, xxx_Symbol, xxxLogo }) {
  const [apr, setApr] = useState(0);
  const [inputEther, setInputEther] = useState(0);
  const [outputEther, setOutputEther] = useState(0);

  const isSwap = farm?.tokens?.[0]?.symbol == "CZB" ? (
    (farm?.tokens?.[1]?.symbol == "CZF") ? true : false
  ...
  useEffect(() => {
    if (!xxxFarmsSettings?.xxxPerSecond || !xxxFarmsSettings?.totalAllocPoint || !lpInfo?.totalSupply || !lpInfo?.tokens[0] || !xxxFarmsPoolInfo?.allocPoint || !xxxFarmsPoolInfo?.totalDeposit || !xxxPrice || !czusdPrice) {
      setApr("0.00");
      return;
    }
    ...
    <CollapsibleCard className="mb-3"
      title={(<div className='has-text-white pb-2 pt-2 '>
        ...
        <CollapsibleCardTitleItem title="TVL" width="4.5em">
          <span className='is-size-6'>${weiToShortString(getLpTokenValueUsdWad(farm?.tokens?.[0]?.symbol ?? xxx_Symbol, lpInfo, xxxFarmsPoolInfo?.totalDeposit, xxxPrice, czusdPrice, isSwap), 1)}</span>
        </CollapsibleCardTitleItem>
        <CollapsibleCardTitleItem title="STAKE" width="4.5em">
          <span className='is-size-6'>${weiToShortString(getLpTokenValueUsdWad(farm?.tokens?.[0]?.symbol ?? xxx_Symbol, lpInfo, xxxFarmsUserInfo?.amount, xxxPrice, czusdPrice, isSwap), 1)}</span>
        </CollapsibleCardTitleItem>
      ...
      {(!!account && lpAllowance?.lt(accountLpBal ?? 0)) && (
        <div>
          <p className='mb-2'>To use this {farm?.tokens?.[0]?.symbol}/{farm?.tokens?.[1]?.symbol} Farms V2, you need to approve the Exotic Master address <code>{address_xxx_master}</code> to use your {farm?.tokens?.[0]?.symbol}/{farm?.tokens?.[1]?.symbol} LP tokens. You can use the button below.</p>
      ...
      {(!!account && lpAllowance?.gte(accountLpBal ?? 0)) && (<>
        <div className="is-flex is-flex-direction-row is-flex-wrap-wrap">
          <div className="is-inline-block p-3 m-3 is-align-self-flex-start " style={{ border: "solid 1px #dbdbdb", maxWidth: "25em" }}>
            <h3 className="is-size-4">Stake {farm?.tokens?.[0]?.symbol}/{farm?.tokens?.[1]?.symbol} LP For {xxx_Symbol}/Second</h3>
            <p>Stake your ${farm?.tokens?.[0]?.symbol}/${farm?.tokens?.[1]?.symbol} LP and get {xxx_Symbol} every second. 1% fee.</p>
            ...
            />
            <p className='is-size-7 mt-0 mb-1 ml-2' >(${weiToShortString(getLpTokenValueUsdWad(farm?.tokens?.[0]?.symbol, lpInfo, parseEther(inputEther.toString()), xxxPrice, czusdPrice, isSwap), 2)})</p>
          ...
          <div className="is-inline-block p-3 m-3 is-align-self-stretch " style={{ border: "solid 1px #dbdbdb", maxWidth: "25em" }}>
            ...
            />
            <p className='is-size-7 mt-0 mb-1 ml-2' >(${weiToShortString(getLpTokenValueUsdWad(farm?.tokens?.[0]?.symbol, lpInfo, parseEther(outputEther.toString()), xxxPrice, czusdPrice, isSwap), 2)})</p>
