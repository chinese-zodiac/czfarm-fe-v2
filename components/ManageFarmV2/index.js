...
export default function ManageFarmV2({
  account,
  library,
  farm,
  v2FarmsSettings,
  ...
  useEffect(() => {
    if (
      !v2FarmsSettings?.czfPerBlock ||
      !v2FarmsSettings?.totalAllocPoint ||
      !lpInfo?.totalSupply ||
      !lpInfo?.tokens[0] ||
      !v2FarmsPoolInfo?.allocPoint ||
      !v2FarmsLpBal?.lpBal ||
      !czfPrice ||
      !czusdPrice
    ) {
    ...
    let usdPerYear = weiToUsdWeiVal(
      v2FarmsSettings.czfPerBlock
        .mul(10519200)
        .mul(v2FarmsPoolInfo.allocPoint)
        .div(v2FarmsSettings.totalAllocPoint),
      czfPrice
    );
    ...
    !v2FarmsSettings?.czfPerBlock || !v2FarmsSettings?.totalAllocPoint,
    lpInfo?.totalSupply,
    lpInfo?.tokens[0],
    v2FarmsPoolInfo?.allocPoint,
    v2FarmsLpBal?.lpBal,
        ...
        title={
          <div className="has-text-white pb-2 pt-2 ">
            ...
            <CollapsibleCardTitleItem title="CZF/DAY" width="4em">
              <span className="is-size-6">
                {weiToShortString(
                  getSingleV2FarmCzfPerSecondWei(
                    v2FarmsSettings,
                    v2FarmsLpBal,
                    v2FarmsPoolInfo,
                    v2FarmsUserInfo
            ...
            <CollapsibleCardTitleItem title="EST CLAIM" width="4em">
              <span className="is-size-6">
                {weiToShortString(v2FarmsPendingCzf?.pendingCzf, 1)}
              </span>
            </CollapsibleCardTitleItem>
          </div>
        }
        ...
        {!!account && lpAllowance?.lt(accountLpBal ?? 0) && (
          <div>
            <p className="mb-2">
              ...
              Exotic Master address <code>{ADDRESS_FARMMASTERV2}</code> to use
              your {farm?.tokens?.[0]?.symbol}/{farm?.tokens?.[1]?.symbol} LP
              tokens. You can use the button below.
            </p>
            <button
              onClick={() => sendApprove(ADDRESS_FARMMASTERV2, MaxUint256)}
            ...
            <div className="is-flex is-flex-direction-row is-flex-wrap-wrap">
              ...
              >
                ...
                <p>
                  ...
                  {farm?.tokens?.[1]?.symbol} LP and get CZF every second. There
              ...
              >
                ...
                <p>
                  Unstake your ${farm?.tokens?.[0]?.symbol}/$
                  {farm?.tokens?.[1]?.symbol} LP. There are no restrictions or
                  fees. Rewards are claimed.
                </p>
                <InputTokenEther
                  className="is-inline-block has-background-special has-text-white is-inline-block mt-2 mb-2"
                  style={{ maxWidth: '10em', width: '100%' }}
                  ...
                  label={`${farm?.tokens?.[0]?.symbol}/${farm?.tokens?.[1]?.symbol}`}
                  minWadBn={BigNumber.from(0)}
                  maxWadBn={v2FarmsUserInfo?.amount ?? BigNumber.from(0)}
                ...
                <QuickInputEther
                  setInputEther={setOutputEther}
                  maxTokenWad={v2FarmsUserInfo?.amount ?? BigNumber.from(0)}
                />
                <button
                  onClick={() =>
                    sendWithdraw(
              ...
              >
                ...
                <p>
                  Harvests your CZF for this farm only and transfers it to your
                  wallet.
                </p>
                <br />
                <button
                ...
                <p>
                  You will get{' '}
                  {weiToShortString(v2FarmsPendingCzf?.pendingCzf, 3)} CZF.
                </p>
              </div>
            </div>
          </>
