...
import { ADDRESS_BANDIT, ADDRESS_CZB, ADDRESS_CZR } from "../constants/addresses";
...
function useLpInfo(library) {
  const lpTokens = {};
  const lpAddresses = [
    ...new Set([...FARM_V2.map(farm => farm.lp), ...EXOTIC_FARMS.map(farm => farm.lp), ...TRIBE_POOLS.map(farm => farm.lp), ...CZB_FARMS.map(farm => farm.lp), ...BANDIT_FARMS.map(farm => farm.lp)])
  ];
  [...FARM_V2, ...EXOTIC_FARMS, ...TRIBE_POOLS, ...CZB_FARMS, ...BANDIT_FARMS].forEach((farm) => {
    if (!lpTokens?.[farm.lp]) {
